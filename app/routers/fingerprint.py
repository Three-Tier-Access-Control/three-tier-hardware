...
def get_fingerprint():
    ...
    mylcd.lcd_display_string("Waiting for fingerprint image...")
    while finger.get_image() != adafruit_fingerprint.OK:
        pass
    print("Templating...")
    mylcd.lcd_display_string("Templating fingerprint image...")
    if finger.image_2_tz(1) != adafruit_fingerprint.OK:
        print("Finger not found")
        mylcd.lcd_display_string("Finger not found")
        raise HTTPException(
    ...
    mylcd.lcd_display_string("Searching for a match...")
    ...
    mylcd.lcd_display_string("Fingerprint match detected")
    return {
        "data": {
...
def enroll_finger(fingerprint: Fingerprint):
    ...
    for fingerimg in range(1, 3):
        if fingerimg == 1:
            mylcd.lcd_display_string("Place finger on sensor...")
            print("Place finger on sensor...", end="", flush=True)
        else:
            mylcd.lcd_display_string("Place same finger again...")
        ...
        while True:
            ...
            if i == adafruit_fingerprint.OK:
                print("Image taken")
                mylcd.lcd_display_string("Fingerprint image taken")
                break
            if i == adafruit_fingerprint.NOFINGER:
                ...
                print("Imaging error")
                mylcd.lcd_display_string("Imaging error")
                # return False
                raise HTTPException(
                ...
                print("Other error")
                mylcd.lcd_display_string("Internal error")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Other error",
                )
        ...
        print("Templating...", end="", flush=True)
        mylcd.lcd_display_string("Templating fingerprint...")

        i = finger.image_2_tz(fingerimg)
        if i == adafruit_fingerprint.OK:
            print("Templated")
            mylcd.lcd_display_string("Templating done!")

        else:
            if i == adafruit_fingerprint.IMAGEMESS:
                print("Image too messy")
                mylcd.lcd_display_string("Fingerprint image too messy")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Image too messy",
                )
            elif i == adafruit_fingerprint.FEATUREFAIL:
                mylcd.lcd_display_string("Could not identify features")
                print("Could not identify features")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                ...
                print("Image invalid")
                mylcd.lcd_display_string("Fingerprint image invalid")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    mylcd.lcd_display_string("Creating model...")

    i = finger.create_model()
    if i == adafruit_fingerprint.OK:
        print("Created")
        mylcd.lcd_display_string("Created...")
    else:
        if i == adafruit_fingerprint.ENROLLMISMATCH:
            print("Prints did not match")
            mylcd.lcd_display_string("Prints did not match")
            raise HTTPException(
    ...
    mylcd.lcd_display_string("Storing model...")
    i = finger.store_model(fingerprint.location)
    if i == adafruit_fingerprint.OK:
        print("Stored")
        mylcd.lcd_display_string("Fingerprint stored")
    else:
        if i == adafruit_fingerprint.BADLOCATION:
            print("Bad storage location")
            mylcd.lcd_display_string("Bad storage location")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            mylcd.lcd_display_string("Flash storage error")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
